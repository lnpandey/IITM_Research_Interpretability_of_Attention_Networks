# -*- coding: utf-8 -*-
"""zeroth-layer-entropy.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/176KwiS3KHYX-Ziq4QvLGfuBNHTqbs-1z
"""
...
n_seed =[0, 1, 2]
lr = [0.0005,0.001]
k = [0,0.001,0.003,0.005] # k 0, 0.001 0.003, 0.005
...
for n_seed_ in n_seed:
    for lr_ in lr:
        for k_ in k:
            ...
            analysis_data_train, sparsity_value_train =calculate_attn_loss(train_loader,classify,focus_net,k_)

            analysis_data_test, sparsity_value_test =calculate_attn_loss(test_loader,classify,focus_net,k_)
            ...
            for epoch in range(nos_epochs):  # loop over the dataset multiple time
                ...
                for i, data in  enumerate(train_loader):
                    ...
                    loss,_,_ = my_cross_entropy(outputs, labels,alphas,log_alphas,k_)
                    loss.backward()
            ...
            torch.save(focus_net.state_dict(),path+"seed_"+str(n_seed_)+"lr_"+str(lr_)+"k_"+str(k_)+"weights_focus.pt")  
            torch.save(classify.state_dict(),path+"seed_"+str(n_seed_)+"lr_"+str(lr_)+"k_"+str(k_)+"weights_classify.pt")

            Analysis_["train_seed_"+str(n_seed_)+"_lr_"+str(lr_)+"k_"+str(k_)] = np.array(analyse_data_train)
            Analysis_["test_seed_"+str(n_seed_)+"_lr_"+str(lr_)+"k_"+str(k_)]  = np.array(analyse_data_test)
            Analysis_["train_sparsity_"+str(n_seed_)+"_lr_"+str(lr_)+"k_"+str(k_)] = np.array(sparsty_train)
            Analysis_["test_sparsity_"+str(n_seed_)+"_lr_"+str(lr_)+"k_"+str(k_)] = np.array(sparsty_test)
...
for seed_ in n_seed:
    for lr_ in lr:
        for  k_ in k:
            ...
            plt.savefig(path + 'seed ' + str(seed_)+" lr " +str(lr_)+"k_"+str(k_)+ "_train.png",bbox_inches="tight")
            plt.savefig(path + 'seed ' + str(seed_)+" lr " +str(lr_)+ "k_"+str(k_)+"_train.pdf",bbox_inches="tight")
            plt.grid()
            plt.show()
...
for seed_ in n_seed:
    for lr_ in lr:
        for  k_ in k:
            ...
            plt.savefig(path + 'seed ' + str(seed_)+" lr " +str(lr_)+"k_"+str(k_)+"_test.png",bbox_inches="tight")
            plt.savefig(path + 'seed ' + str(seed_)+" lr " +str(lr_)+"k_"+str(k_)+ "_test.pdf",bbox_inches="tight")
            plt.grid()
            plt.show()
